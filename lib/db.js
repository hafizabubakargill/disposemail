 ...
 */
async function persistToMongo(emailData, retries = 2) {
  try {
    const exists = await Email.findOne({ id: emailData.id }).maxTimeMS(4000);
    if (!exists) {
      await Promise.race([
    ...
    if (retries > 0) {
      setTimeout(() => persistToMongo(emailData, retries - 1), 3000);
    }
  }
}
