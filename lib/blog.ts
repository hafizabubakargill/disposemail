  ...
  es: [
    {
      ...
      image: '/blog/privacy.png',
      content: `
        <p>Tu dirección de correo electrónico principal es más que una simple herramienta de comunicación; es un pasaporte digital. Está vinculada a sus cuentas bancarias, perfiles de redes sociales e identidad profesional. Cuando comparte esta dirección con cada sitio web que visita, está dejando un rastro de puntos de datos vulnerables a través de Internet.</p>
        
        <h3>El riesgo de la estrategia de "un solo correo"</h3>
        <p>La mayoría de los usuarios utilizan un único correo electrónico para todo. Si bien es conveniente, esto crea un punto único de falla. Si un sitio web menor sufre una violación de datos, su correo electrónico principal estará ahora en manos de actores maliciosos.</p>
        
        <h3>Cómo le protegen los correos desechables</h3>
        <p>Los servicios de correo electrónico desechable como DisposeMail le permiten crear un "cortafuegos" entre su verdadera identidad y los sitios web con los que interactúa. Al utilizar una dirección temporal para registros, boletines y pruebas, se asegura de que, incluso si ese servicio se ve comprometido, su bandeja de entrada real permanezca intacta e invisible.</p>
      `
    },
    {
      slug: 'dangers-reusing-email-social-media',
...
['pt', 'ru', 'zh'].forEach(lang => {
  blogPosts[lang] = blogPosts.en.map(post => ({ ...post }));
});
...
blogPosts.en.forEach(enPost => {
  if (!blogPosts.es.find(esPost => esPost.slug === enPost.slug)) {
    blogPosts.es.push({ ...enPost });
  }
});

export function getSortedPosts(locale: string = 'en') {
