...
export default function SecureNotesCreator() {
    ...
    return (
        <div className="w-full max-w-3xl mx-auto backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl shadow-2xl p-6 sm:p-10 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center mb-8 text-center">
                ...
                <h2 className="text-3xl font-bold text-white mb-2">{t('creatorTitle')}</h2>
                <p className="text-gray-400 max-w-md">{t('creatorSubtitle')}</p>
            </div>
            ...
            {status === 'idle' || status === 'loading' || status === 'error' ? (
                <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                    <div className="relative">
                        <textarea
                            ...
                            className="w-full h-48 md:h-64 p-5 bg-black/40 border border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-gray-200 placeholder-gray-600 resize-none transition-all font-mono custom-scrollbar"
                            required
                            disabled={status === 'loading'}
                        />
                        <div className="absolute bottom-4 right-4 flex items-center text-xs text-gray-500 gap-1 bg-black/80 px-2 py-1 rounded-md border border-white/5">
                ...
                <div className="flex flex-col items-center justify-center py-8 animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="w-full bg-black/50 border border-indigo-500/30 rounded-xl p-2 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-8">
                        <div className="flex-1 flex items-center px-4 py-3 bg-black/40 rounded-lg overflow-hidden relative group">
                            <LinkIcon className="w-4 h-4 text-indigo-400 mr-3 flex-shrink-0" />
                            <span className="text-indigo-300 font-mono text-sm truncate select-all">{shareUrl}</span>
                        </div>
                        <button
